...
public class BubbleSprite extends Sprite {
	...
	/**
	 * GetTypeId.
	 * Return sprite type - which is TYPE_BUBBLE.
	 * 
	 ...
	 * @param frozen
	 */
	public BubbleSprite(Rect area, int direction, int color,
	...
	public void move() {
		...
		Vector<BubbleSprite> neighbors = getNeighbors(currentPosition);
		...
		if (checkCollision(neighbors) || realY < 44. + frozen.getMoveDown()) {
			...
			Vector<BubbleSprite> checkJump = new Vector<BubbleSprite>();
			...
			if (checkJump.size() >= 3) {
				...
				for (int i = 0; i < checkJump.size(); i++) {
					BubbleSprite current = (BubbleSprite) checkJump
							.elementAt(i);
	 ...
	 * 
	 * @param p Point
	 * @return Vector<BubbleSprite> neighbors
	 */
	 ...
	 *            BmpWrap
	 */
	void checkJump(Vector<BubbleSprite> jump, BmpWrap compare) {
		if (checkJump) {
			return;
		}
		checkJump = true;
	 ...
	 *            Vector
	 */
	void checkJump(Vector<BubbleSprite> jump, Vector<BubbleSprite> neighbors) {
