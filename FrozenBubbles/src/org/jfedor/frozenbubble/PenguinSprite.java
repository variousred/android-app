...
/**
 * PenguinSprite. 
 * Paints the penguin handling the launcher.
 * Everything the penguin does is based on aiming left or right, firing, or won or lost.
 * 
 */
public class PenguinSprite extends Sprite {
	...
	public final static int[][] LOST_SEQUENCE = { { 1, 0 }, { 2, 8 }, { 3, 9 },
			{ 4, 10 }, { 5, 11 }, { 6, 12 }, { 7, 13 }, { 5, 14 } };
	public final static int[][] WON_SEQUENCE = { { 1, 0 }, { 2, 7 }, { 3, 6 },
			{ 4, 15 }, { 5, 16 }, { 6, 17 }, { 7, 18 }, { 4, 19 } };
	...
	/**
	 * PenguinSprite. Penguin initialization.
	 * 
	 * @param sprites
	 *            BmpWrap
	 * @param rand
	 ...
	 *            int
	 */
	public PenguinSprite(BmpWrap sprites, Random rand, int currentPenguin,
			int count, int finalState, int nextPosition) {
		super(new Rect(361, 436, 361 + 55, 436 + 43));
	 ...
	 * saveState.
	 * 
	 * @see org.jfedor.frozenbubble.Sprite#saveState(android.os.Bundle,
	 *      java.util.Vector)
	 */
	...
	/**
	 * getTypeId. Returns sprite type - TYPE_PENGUIN
	 * 
	 * @see org.jfedor.frozenbubble.Sprite#getTypeId()
	 */
	...
	/**
	 * updateState. Sets penguin to next position.
	 * 
	 * @param state
	 *            int
	 */
	...
	/**
	 * paint. Paints penguin based on current state.
	 * 
	 * @see org.jfedor.frozenbubble.Sprite#paint(android.graphics.Canvas,
	 *      double, int, int)
	 */
	public void paint(Canvas c, double scale, int dx, int dy) {
		Rect r = this.getSpriteArea();
		drawImageClipped(spritesImage, 360 - (currentPenguin % 4) * 57,
				435 - (currentPenguin / 4) * 45, r, c, scale, dx, dy);
	}
}
